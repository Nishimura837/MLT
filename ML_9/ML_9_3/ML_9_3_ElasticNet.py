...
def objective(trial):
    ...
    l1_ratio = trial.suggest_float('l1_ratio', 0.1, 0.9, step=0.1)
    ...
    for train_index, val_index in kf.split(X_train):
        ...
        model.fit(X_poly_train_fold, y_train_fold)

        X_poly_val_fold = poly.transform(X_val_fold)
        y_pred = model.predict(X_poly_val_fold)
...
y_train_pred = elasticnet.predict(X_train_poly)
y_test_pred = elasticnet.predict(X_test_poly)
...
df_er.to_csv(
    "/home/gakubu/デスクトップ/MLTgit/MLT/ML_9/ML_9_3/df_er_ElasticNet.csv", index=False)
...
plt.axhline(y=0, c='k')
plt.legend()
plt.savefig("/home/gakubu/デスクトップ/MLTgit/MLT/ML_9/ML_9_3/ML_9_3_ElasticNet.png")
plt.show()
