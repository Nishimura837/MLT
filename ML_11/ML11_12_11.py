...
# 誤分類した画像の枚数を確認、誤分類した画像を表示 (ホンマにちゃんとできた)
# まで完了
...
def create_model():
    ...
    conv2D = Conv2D(32, (kernel,kernel), padding="same", activation="relu")(x)
...
misclassified_indices = np.where(y_test_pred_labels != y_test)[0]
np.savetxt("/home/gakubu/デスクトップ/ML_git/MLT/ML_11/data.csv", misclassified_indices, delimiter=",")
# 表示する画像の枚数
...
# intermediate_layer_model = Model(inputs=model.input, outputs=model.get_layer("conv2d").output)  # Flatten レイヤーの手前の層
...
# # プロット
# scatter = plt.scatter(tsne_result[:, 0], tsne_result[:, 1], c=y_train, cmap="tab10")
